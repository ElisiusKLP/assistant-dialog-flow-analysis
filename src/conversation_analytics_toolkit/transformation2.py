...
def to_canonical_WA_v2(df, wa_skills=None, skill_id_field=None, include_nodes_visited_str_types=False, include_context=False):
    ...
    ### CUSTOM CODE HERE ###
    #expand global_context columns (include fields e.g. the output text)
    df3 = pd.concat([df2.drop(['response_context_global'], axis=1), 
        pd.DataFrame(df2['response_context_global'].tolist()).add_prefix('response_context_global_')], axis=1)

    #expand global_context_system columns
    df10 = pd.concat([df3.drop(['response_context_global_system'], axis=1), 
        pd.DataFrame(df3['response_context_global_system'].tolist()).add_prefix('response_context_global_system_')], axis=1)

    #expand response_output_debug columns (e.g. include fields branch_exited and exited_reason)
    df11= pd.concat([df10.drop(['response_output_debug'], axis=1), 
        pd.DataFrame(df10['response_output_debug'].tolist()).add_prefix('response_output_debug_')], axis=1)

    df11.rename(columns={'response_output_debug_branch_exited': 'response_context_system_branch_exited',
                        'response_output_debug_branch_exited_reason': 'response_context_system_branch_exited_reason'}, inplace=True)

    #expand response output generic columns (include fields e.g. the output text)  
    if 'response_output_generic' in df11:
    ...
    if skill_id_field != None:
        ...
        'response_output_intents',skill_id_field]
        #'response_output_generic_response_type']
    else: 
        cols = ['response_context_conversation_id', 'log_id' ,'response_timestamp', 
        'request_input', 'response_output_text' ,'response_output_nodes_visited', 
        'response_context_system_branch_exited', 'response_context_system_branch_exited_reason',
        'response_output_intents',"workspace_id"]
        #'response_output_generic_response_type']
    ...
    if skill_id_field != None:
        df5.rename(columns={'response_output_nodes_visited': 'nodes_visited',
                        ...
                        'response_context_system_branch_exited' : 'branch_exited',
                        'response_context_system_branch_exited_reason' : 'branch_exited_reason',
                        'response_output_intents' : 'intent',
